...
在「表单管理」中可对每个表单开启两类防护：

- **提交需图形验证码**：提交前完成点选/滑块验证，拦截机器人最有效
- **提交需签名挑战**：提交需携带页面脚本获取的短时效令牌，拦截直接 POST 接口的脚本提交

开启任一防护后，页面引入组件脚本即可自动生效（组件自动识别 `/f/{token}` 表单并生成隐藏字段）：

```html
<script src="https://formail.it0731.cn/assets/js/formail-captcha.js" defer></script>
```

两种防护均未开启时，表单不拦截任何提交，垃圾邮件会发送到该表单收件邮箱（服务端仍有内容过滤、字段长度/数量上限、来源白名单与频率限制兜底）。
